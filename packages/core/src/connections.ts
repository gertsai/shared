 ...
 */
export const CONNECTION_COMPATIBILITY: Record<GertsConnectionType, readonly GertsConnectionType[]> = {
  // Main accepts any
...
export function validateConnection(
  sourceType: GertsConnectionType,
  targetType: GertsConnectionType
 ...
 */
export function isAiConnectionType(type: GertsConnectionType): boolean {
 ...
 */
export function isGraphConnectionType(type: GertsConnectionType): boolean {
  return (
 ...
 */
export function getAcceptedConnectionTypes(
  targetType: GertsConnectionType
): readonly GertsConnectionType[] {
...
export function getCompatibleTargets(
  ...
  const compatibleTargets: GertsConnectionType[] = [];

  for (const [target, accepted] of Object.entries(CONNECTION_COMPATIBILITY)) {
    if ((accepted as readonly string[]).includes(sourceType)) {
      compatibleTargets.push(target as GertsConnectionType);
    }
  }
