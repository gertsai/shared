...
export class ModelRouter {
     ...
     */
    createWithFallback(models, config) {
        if (models.length === 0) {
     ...
     */
    getCostOptimizedModel(taskType, provider = this.defaultProvider) {
        const modelMap = {
            openai: {
                simple: 'gpt-4o-mini',
                complex: 'gpt-4o',
     ...
     */
    createProvider(provider, model, config) {
        const baseConfig = {
            model,
