...
export class HookExecutor {
     ...
     */
    async executePreHooks(hooks, params, runInBackground = false) {
     ...
     */
    async executePostHooks(hooks, params, runInBackground = false) {
        const sorted = this.sortByPriority(hooks);
        for (const hook of sorted) {
            const shouldRunInBg = shouldRunInBackground(hook) || runInBackground;
            const hookName = getHookName(hook);
     ...
     */
    async executeToolPreHooks(hooks, fc) {
        for (const hook of hooks) {
            try {
     ...
     * @returns Modified result or null
     */
    async executeToolPostHooks(hooks, fc) {
        let modifiedResult = null;
        for (const hook of hooks) {
            try {
                const result = await hook(fc);
    ...
    normalizeHooks(hooks, type) {
        ...
        return hooks.map((hook) => {
            ...
            // Guardrail
            if ('check' in hook && typeof hook.check === 'function') {
                return hook.check.bind(hook);
            }
            // Evaluator
            const evaluator = hook;
     ...
     */
    emitHookEvent(eventType, hookName, hookType, isBackground, durationMs, error) {
        const event = {
            type: eventType,
            timestamp: new Date(),
            hookName,
