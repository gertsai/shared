...
export abstract class BaseQueryExecutor<
  ...
  TData,
  TMeta = unknown
> implements IQueryExecutor<TQuery, TData, TMeta>
{
  /**
   ...
   */
  async *stream(query: TQuery): AsyncIterable<QueryResult<TData, TMeta>> {
    yield await this.execute(query);
  }
  ...
  ): Promise<QueryResult<TData, TMeta>> {
    try {
      return await fn();
    } catch (error) {
  ...
  protected validateQueryType(query: TQuery): QueryFailure | null {
    if (!this.supportsType(query.type)) {
      return queryFailure(
        'INVALID_QUERY',
        `Query type '${query.type}' not supported by ${this.metadata.name}. ` +
          `Supported: ${this.metadata.supportedTypes.join(', ')}`,
        { retryable: false }
      );
    }
    return null;
...
export type ExecutorFactory<
  ...
  TMeta = unknown,
  TConfig = unknown
> = (config: TConfig) => IQueryExecutor<TQuery, TData, TMeta>;

/**
 * Executor registry entry
 */
export interface ExecutorRegistryEntry<
  TQuery extends QueryRequest = QueryRequest,
  TData = unknown,
  TMeta = unknown
> {
  /** Executor instance */
 ...
 */
export function createExecutorMetadata(
  name: string,
  options: Omit<ExecutorMetadata, 'name'>
): ExecutorMetadata {
  return {
    name,
    ...options,
