...
## Task List Discipline — дисциплина списка задач

Список задач (`TaskCreate` / `TaskUpdate` / `TaskList`) — **единственная долговечная запись
работы в полёте**. Чат прокручивается, субагенты уходят в простой, контекст сжимается —
всё, что живёт только в прозе, теряется. Дополняет Forgeplan (источник правды по *решениям*,
см. выше) и `tasks-manifest.json`; здесь — про *исполнение* работы.

**Базовое.** Единица работы становится задачей **до** того, как кто-то трогает код, а не
после. Каждая задача несёт владельца — имя субагента (из `Agent`-блока) или главную сессию.
Задача `in_progress` без владельца = работа, которую никто не делает. Для forgeplan-артефактов
владение фиксируется отдельно через `forgeplan claim <ID> --ttl-minutes N` / `release`
(§Multi-agent).

**Статусы честные.** `in_progress` ставится **при старте**, не при планировании.
Заблокированная задача **остаётся `in_progress`** и получает пометку о блокере — не
откатывается тихо в `pending`. `completed` означает, что **верификацию перезапустил ты сам**
(`/quality-gate --scope=<workspace>`, `pnpm --filter <pkg> {test,typecheck,lint}`,
`gh pr checks`, skill `verification-loop`), а не что субагент отрапортовал об успехе.
Перезапуск дёшев, ложный зелёный — нет.

**Ничего не теряется.** Находки по ходу задачи становятся **новыми задачами**, а не строчками
в отчёте: «заметил X, пока делал Y» и больше нигде — ровно так X и теряется. Отложенная работа
получает задачу с **явным триггером** в форме ADR: *что должно стать правдой, чтобы её взяли*.
«Потом» — не триггер. Описание задачи хранит **что решили и почему, включая исправления**:
если заявленная острота находки оказалась завышенной, исправленная версия живёт в задаче,
чтобы следующий читатель не вывел заново неверный вывод.

**Переоткрывать без колебаний.** Субагент, помечающий свою задачу готовой, выносит
**утверждение, а не вердикт**. Нет доказательства — негативный/мутационный тест не
компилировался, гейт (`/quality-gate`, CI) не запускался, отчёт не пришёл — верни в
`in_progress` и **назови, какого именно доказательства не хватает**.

**Параллельные агенты.** Строгое владение файлами объявляется в задании (план от
`forgeplan dispatch --agents N` даёт conflict-free buckets): перечисли, какими файлами агент
владеет и какие трогать не должен. Реально нужно пересечь границу — агент **останавливается и
спрашивает**, а не правит и упоминает потом. В файлах, где может быть другой агент, — только
**точечные правки `Edit` по совпадению строки, никогда `Write` целого файла**:
непересекающиеся `Edit` сосуществуют, целофайловая запись молча съедает чужое изменение.
Субагенты уходят в простой, не отправив отчёт — **проверяй дерево сам** (`git status` /
`git diff`), это быстрее и надёжнее повторного запроса. Негативный/мутационный тест оставляет
в рабочей копии настоящую уязвимость: держи окно коротким, не уходи в простой с ней, проверяй
откат самостоятельно.

**Чего не делать.** Не заводить задачу на однострочную правку (это Tactical — см. выше). Не
держать задачу открытой «на всякий случай». Не дублировать в задаче то, что уже записано в
